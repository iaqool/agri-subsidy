...
| `SENTRY_DSN` | Sentry DSN. Empty → Sentry disabled, agent boots without contacting Sentry. When set, `agent/monitoring.py:init_sentry` runs at `@app.on_event("startup")` with `traces_sample_rate=0` (error-only, no perf spans) and `send_default_pii=False` (no request bodies → no wallet leakage). FastAPI integration auto-captures unhandled exceptions. | unset |
| `SENTRY_ENVIRONMENT` | Tag on Sentry events. Use `production` on Railway / fly.io, `staging` on previews, `local` for dev. | `local` |
| `DISCORD_WEBHOOK_URL` | Discord incoming-webhook URL. Empty → no alerts. When set, the agent fires fire-and-forget alerts (`asyncio.create_task`, 3s timeout, never raises) on (1) degraded-MOCK fallbacks from `release_subsidy` and (2) unhandled exceptions in the eval pipeline. Webhook URL is a credential — never commit. Note: no rate-limiting yet, so a sustained Devnet outage can produce one alert per evaluation. | unset |
| `DISCORD_ALERT_USERNAME` | Display name on the webhook (visible in the Discord channel). | `Dala Alert` |
